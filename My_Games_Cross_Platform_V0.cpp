...
// The raw Windows scan-codes for Home/End/Arrow keys are 71/79/75/77/72/80,
// which are numerically IDENTICAL to the uppercase ASCII letters G/O/K/M/H/P.
// That is exactly why pressing those Capital Letters used to be confused
// with pressing Home/End/Arrow keys. Offsetting them by +1000 makes every
// one of these constants impossible to match with ANY normal character
// (0-255), so the conflict is gone for good. ReadKey() (below, near
// Key_Hit_Maintainor) is what actually produces these safe values —
// identically on Windows, Linux, Mac, and Android/Termux.
...
#define Border_Sign          if(Marker==2) cout<<char(219); else if(!Snake_Boundary) {cout<<"."; if(!Joined_Single_Snake) cout<<" "; } else { cout<<char(219); if(!Joined_Single_Snake) cout<<char(219);};
#define Multiple_Border_Sign if(Marker==2) {for(int i=0;i<21;i++) cout<<char(219);} else if(!Snake_Boundary) { for(int i=0;i< Snake_Area_Column + 1; i++) Border_Sign }      else { for(int i=0;i<Snake_Area_Column +1;i++) Border_Sign }
...
void fontManager()
{
        ...
        SetConsoleOutputCP(437);   // CP437 force করবে, smiley guaranteed দেখাবে
        SetConsoleCP(437);
...
void Instruction()
{
    ...
    cout<< "Press -->  Up/Down/Left/Right Arrow Keys  : to Control Game in Gaming Time     \n";
    cout<< "Press -->       Up/Down       Arrow Keys  : to Mark the Game in Menu      \n";
    cout<< "Press -->      Enter/ Right_Arrow Key     : to Entered into the Marked Game in Menu      \n\n";
...
//    cout<<"[FIXED] Capital Letters no longer conflict with Arrow / Home / End Keys.\n\n";

    SetConsoleColour (Black,BrightWhite);
    cout<<"Press -->  < Small 'f' >, to Refresh the Screen\n";
...
void Snake_Instruction()
{
    ...
    Multi_Tabs_1 cout<<"To Refresh Screen     -> Press <      Enter-Key     >"<<endl;
...
bool Snake_Premium=true, DoubleSnake = true, Ultra_Premium_Protector = true, Joined_Single_Snake=false;
bool Snake_Boundary=false, Self_Hit=true;
bool Pac_Premium = false;
...
void Show_Main_Menu()
{
    ...
    if(KEY=='*') this_thread::sleep_for(chrono::milliseconds(10*Time_Delay));

    if(KEY!=DOWN_KEY && KEY!=UP_KEY) clearScreen(); //All Previous Print Erased
    else                             gotoxy(0,0);
    ...
    Multi_Tabs_3                                           cout<<char(32); printf(" Press   ?  :   to See                MORE INSTRUCTIONS\n");
    Multi_Tabs_3                                           cout<<char(32); printf(" Press   *  :   to See                             DEMO\n\n");

    Multi_Tabs_3                                           cout<<char(32); printf(" Press ENTER :  to Enter                the MARKED GAME\n");
    Multi_Tabs_3                                           cout<<char(32); printf(" Press  END  :  to CLOSE                    THE PROGRAM\n\n");
    ...
    while(true)
    {
        ...
        if (kbhit())           //Control by arrow KEY
        {
            ...
            else if(KEY=='*')
            {
                ...
                for(int i=0;i<18;i++) {Time_Delay++; if(Time_Delay>100) Time_Delay=91; if(Time_Delay>10) Time_Delay+=9; cout<<"\rPolling_Delay(ms): "<<Time_Delay<<"                          ";this_thread::sleep_for(chrono::milliseconds(10));}
                for(int i=0;i<19;i++) {Time_Delay--; if(Time_Delay<0  ) Time_Delay=0;  if(Time_Delay>10) Time_Delay-=9; cout<<"\rPolling_Delay(ms): "<<Time_Delay<<"                          ";this_thread::sleep_for(chrono::milliseconds(10));}
                for(int i=0;i<1;i++)  {Time_Delay++; if(Time_Delay>100) Time_Delay=91; if(Time_Delay>10) Time_Delay+=9; cout<<"\rPolling_Delay(ms): "<<Time_Delay<<"                          ";this_thread::sleep_for(chrono::milliseconds(10));}
            ...
            while(true)
            {
                ...
                while(true)
                {
                    ...
                    while(true)
                    {
                        ...
                        if(Snake_Premium && !Ultra_Premium_Protector)   PlayGround [Bonus] = 5  + Step%2;
                        else if(Snake_Premium)     PlayGround [Bonus] =  176 + Step%2;
                        else                    PlayGround [Bonus] = '$' - Step%2;

                        PlayGround [Fruit] = Step%2+'*'; //Fruit Symbol (*,+)
                        ...
                        for(i = Score+3; i>0; i--)
                        {
                            ...
                            if     ( Snake_Premium && !DoubleSnake)  PlayGround [Snake_Body[i]]  =  254; //Snake Body Symbol
                            if     ( Snake_Premium &&  DoubleSnake)  PlayGround [Snake_Body[i]]  =  219; //Snake Body Symbol
                        }

                        if(Snake_Premium && !Ultra_Premium_Protector) PlayGround [Snake_Body[Score+3]] = 4;    //Snake Tail Symbol
                        else           PlayGround [Snake_Body[Score+3]] = '-';

                        if(Snake_Premium && !Ultra_Premium_Protector) PlayGround [Snake_Head] = 2;    //Snake Head Symbol (Ascii 2)
                        ...
                        if(Snake_Marker==4) {Multi_Tabs_1 cout << "    SNAKE GAME [BOX-N0-SELF-HIT]  \n\n";}
        ...
        case 2:
            ...
            while(true)
            {
                ...
                while(true)
                {
                    ...
                    char Move;
                    bool Fruit[111];
                    char PlayGround[111];                    /// Playground Size 10*10=100 (0-99) + extra 10(just for avoiding silly risk
                    ...
                    while(true)                                        ///Protibar Move korar por Program ekhane chole asbe
                    {
                        ...
                        if(Pac_Premium) PlayGround[PacMan] = 1;          ///PacMan Symbol (Smile face Black)
                        else           PlayGround[PacMan] ='O';

                        for(i=0;i<Number_of_Enemy;i++)
                        {
                            if(Pac_Premium) PlayGround[Enemy[i]] = 2;                   ///Enemy Symbol (Smile face White)
        ...
        case 6: //Sudoku (Fixed Given Value)
            ...
            while(true)
            {
                ...
                while(true)
                {
                    ...
                    Multi_Tabs_3 cout << "* Indicates                              Your Selected Index\n";
                    Multi_Tabs_3 cout << "Number   with  `                 Indicates Your Filled Index\n";
                    Multi_Tabs_3 cout << "Number without ` Indicates  Given Index (which Not Changable)\n\n";
                    Multi_Tabs_3 cout << "Press --> Arrow Keys                       to Select   Index\n";
                    Multi_Tabs_3 cout << "Press --> Number Keys (1 to 9)             to Fill the Index\n";
                    Multi_Tabs_3 cout << "Press --> 0                      to Unfill your Filled Index\n";
                    Multi_Tabs_3 cout << "Press --> Capital 'X' / 'END KEY'          to close the Game\n";
                    ...
                    while(true)
                    {
                        ...
                        if (kbhit())           //Control by arrow KEY
                        {
                            ...
                            if    (KEY==UP_KEY)  {row--;break;}
                            if    (KEY==DOWN_KEY)  {row++;break;}
                            if    (KEY==LEFT_KEY)  {col--;break;}
                            if    (KEY==RIGHT_KEY)  {col++;break;}
